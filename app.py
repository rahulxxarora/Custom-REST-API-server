...
class TemporaryDatabase(object):
    items = {'foo': {'id': 'foo', 'seller': 'bar', 'price': '99'}, }

    users = {'root': 'cm9vdDpwd2Q='}
        ...
        def wrapper(self):
            ...
            if self.path == '/':
                ...
                self.wfile.write(
                    bytes(
                        json.dumps({
                            'msg':
                            'Visit /products for more (with proper Auth headers)'
                        })))
            ...
            try:
                if self.headers['Authorization'] and self.headers['user']:
                    if TemporaryDatabase.users.has_key(
                            self.headers['user']
                    ) and self.headers[
                            'Authorization'] == 'Basic ' + TemporaryDatabase.users[
                                self.headers['user']]:
                        return f(self)
                    else:
